...
def test_returns_source_connector():
    with patch("model_ledger.connectors.github.httpx") as mock_httpx:
        ...
        c = github_connector(name="test", repos=["org/repo"], project_path="projects",
                            config_file="config.yaml", parser=_parser)
...
def test_discovers_projects():
    with patch("model_ledger.connectors.github.httpx") as mock_httpx:
        ...
        c = github_connector(name="pipelines", repos=["org/repo"],
                            project_path="projects", config_file="config.yaml",
                            parser=_parser)
        nodes = c.discover()
        assert len(nodes) == 2
...
def test_parser_returns_none_skips():
    ...
    with patch("model_ledger.connectors.github.httpx") as mock_httpx:
        ...
        c = github_connector(name="test", repos=["org/repo"],
                            project_path="projects", config_file="config.yaml",
                            parser=skip_parser)
        assert c.discover() == []
...
def test_auth_header():
    with patch("model_ledger.connectors.github.httpx") as mock_httpx:
        ...
        c = github_connector(name="test", repos=["org/repo"],
                            project_path="projects", config_file="config.yaml",
                            parser=_parser, token="ghp_secret")
        c.discover()
